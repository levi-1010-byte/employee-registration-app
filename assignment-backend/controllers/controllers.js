...
const getallEmployees = async (req, res) => {
    try {
      const result = await db.query('SELECT * FROM "employeeDatatable"."employeeDatatable"');
  ...
  const addEmployee = async (req, res) => {
    try {
     ...
     if (error) {
      ...
      const query = 'INSERT INTO "employeeDatatable"."employeeDatatable" (name, email, department, position, start_date) VALUES ($1, $2, $3, $4, $5) RETURNING *';
  ...
  const updateEmployee = async (req, res) => {
    ...
    if (error) {
      ...
      const query = 'UPDATE "employeeDatatable"."employeeDatatable" SET name = $2, department = $3, email = $4, position = $5, start_date = $6 WHERE id = $1 RETURNING *';
  ...
  const deleteEmployee = async (req, res) => {
    ...
    try {
      ...
      const query = 'DELETE FROM "employeeDatatable"."employeeDatatable" WHERE id = $1 RETURNING *';
      const values = [id];
